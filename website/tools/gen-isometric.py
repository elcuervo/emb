...
K = 60 / 197.0          # dimetric ratio
HW = 197                # plate half-width (plan square side == HW)
...
parts = []
parts.append('<svg class="pipeline__svg" viewBox="0 0 394 %d" aria-hidden="true" focusable="false">' % round(H))
parts.append("""  <defs>
    <pattern id="p-dots" width="34" height="34" patternUnits="userSpaceOnUse" patternTransform="matrix(1 .3046 -1 .3046 0 0)">
    ...
    <pattern id="p-grid" width="20" height="20" patternUnits="userSpaceOnUse" patternTransform="matrix(1 .3046 -1 .3046 0 0)">
      <path d="M0 0H20M0 0V20" fill="none" stroke="#0B0B0B" stroke-width="1.1"/>
    </pattern>
    <pattern id="p-fine" width="11" height="11" patternUnits="userSpaceOnUse" patternTransform="matrix(1 .3046 -1 .3046 0 0)">
      <path d="M0 0H11M0 0V11" fill="none" stroke="#F3F0E8" stroke-width=".7"/>
    </pattern>
    <linearGradient id="ramp" x1="0" y1=".62" x2="1" y2=".06">
      <stop offset="0" stop-color="#000"/><stop offset=".34" stop-color="#000"/>
      <stop offset=".88" stop-color="#fff"/><stop offset="1" stop-color="#fff"/>
    </linearGradient>
    <mask id="ramp-mask"><rect x="0" y="0" width="394" height="%d" fill="url(#ramp)"/></mask>
    <g id="cube">
...
for i, (name, label, cfg) in enumerate(labels):
    ...
    if name == "inference":
        ...
        parts.append('          <path d="%s" fill="url(#p-fine)" opacity=".6"/>' % face_t)
        parts.append('        </g>')
    if name == "embedding":
        parts.append('        <g class="cube-field">')
        for c in cubes(ox, oy, 42, 20260911):
            parts.append("          " + c)
        parts.append('        </g>')
    if name == "serve":
        parts.append('        <path d="%s" fill="url(#p-fine)" opacity=".6"/>' % face_t)
    ...
    parts.append('        <text class="plate-label%s" x="28" y="165" transform="matrix(1 .3046 -1 .3046 %d %d)">%s</text>'
                 % (" plate-label--light" if name == "serve" else "", ox, oy, label))
    parts.append('        <ellipse class="port" cx="%d" cy="%s" rx="11" ry="4.2"/>' % (ox, n(oy + HW * K)))
    parts.append('      </g>')

parts.append('')
parts.append('      <!-- the signal rides ON TOP of the stack -->')
parts.append('      <line class="sig" x1="197" y1="-520" x2="197" y2="548" pathLength="1"/>')
parts.append('</svg>')
...
sky = SKYLINE
CX, CY, CW, CH = 500, 550, 2100, 870   # frame of mountain.jpg used on the page
