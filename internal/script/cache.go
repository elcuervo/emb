...
// "#<sha256(text)>" when it exceeds CacheKeyInlineLimit. The argument hash
// folds the argument count and each argument's length, so ARGV boundaries are
// unambiguous: nil, [""], and ["a","b"] vs ["a\x00b"] all hash differently.
// ARGV is caller-controlled, so the digest uses SHA-256 (a SHA-1 collision
// could replay a cached reply for a different argument sequence); the
