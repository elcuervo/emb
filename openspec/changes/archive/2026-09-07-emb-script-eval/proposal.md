...
- **Whitelisted host functions**: `emb.run`, `emb.tokenize` (`pretokenized`, `words`, `encode`, `encode_pair`), `emb.math` (`sigmoid`, `softmax`, `argmax`), `json`, safe stdlib subset (no `io`/`os`/network/FFI/`math.random`) — scripts are pure compute, so replies are deterministic and cacheable. Cross-model composition (`emb.embed` of another model) is deferred.
...
- **Math baseline (`emb.math`)**: `sigmoid`, `softmax` (stable), `argmax` — the common post-processing primitives, so classification/QA/reranker scripts don't reimplement them. Vectorized (array in → array out) so hot per-candidate loops like GLiNER's span scan pay one boundary crossing per label, not per candidate; scalars accepted for convenience.
- **Plain + pair tokenization**: `emb.tokenize.encode(text, n)` and `emb.tokenize.encode_pair(a, b, n)` using the model tokenizer's own pipeline (correct for byte-level-BPE models where word-splitting differs) with per-token char offsets and `sep` position — enough for classification, extractive QA (span slicing without a decode block), token classification, and cross-encoder rerankers. BERT-family pair template (`[CLS] a [SEP] b [SEP]`); other templates remain script-composable via `encode` + explicit separator ids.
- **Generic building blocks, not maintained adapters**: the server ships reusable host primitives; model-specific glue stays in user scripts. `examples/scripts/` demonstrates the pattern across model families (GLiNER span extraction, SST-2 classification, squad QA, bge reranker).
