...
surface, one demos gallery, one not-found page, and the assets those four
reference. Files that exist to author, measure, or critique the site MUST NOT be
reachable at any path on the deployed origin, and neither MUST the source,
configuration, or presets of a service runtime that happens to live under the
site directory. Because those files may live beside the pages they describe,
their absence MUST NOT rest on convention: the exclusion MUST be declared in the
repository, applied by the deploy tooling, and asserted against the resulting
...
long-term, and a generated artifact whose bytes change under a stable name MUST
either carry a content hash or be revalidated. Every served response MUST carry
an explicit caching policy rather than relying on a platform default that could
change.
...
- **WHEN** a font, a generated image, a vendored player, or a hashed index is requested
- **THEN** the response permits long-lived caching

#### Scenario: A rebuilt index is not served stale

- **WHEN** a demo's vector index is rebuilt with different bytes
- **THEN** a returning visitor receives the new index, because the artifact either carries a content hash in its name or is revalidated
