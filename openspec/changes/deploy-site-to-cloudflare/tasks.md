...
- [x] 5.3 Stop spending the server's and the gems' CI minutes on a change that cannot touch them. Add a `changes` job to `.github/workflows/ci.yml` that classifies the diff — `base...head` against the pull request's own head, `github.event.before..github.sha` on a push — and gate `lint`, `test`, and `gem-rails` on `needs.changes.outputs.code == 'true'`, where `code` means "any path outside `website/`". Use a job-level `if`, never a workflow-level `paths-ignore`: a path-filtered workflow leaves its checks `Pending`, which blocks a merge that requires them, while a conditionally skipped job reports success. Verify `site` stays unconditional, and that a base that cannot be resolved (all-zero `before`, absent base, failing diff) resolves to `true` rather than to a skip.
- [x] 5.4 Verify the classifier against real revisions before trusting it. Extract the job's `run` block and drive it with `EVENT`/`BEFORE`/`PR_BASE`/`PR_HEAD`/`SHA` from this repository's history: a `website/`-only commit (`website: the print rule missed the section heading bars`, `Remove corrections`) yields `code=false`; `website/` plus `DESIGN.md`, a workflow file plus `website/`, and an openspec-only commit each yield `true`; an empty diff yields `false`; and an all-zero `before`, a dispatch with no `before`, and a nonexistent SHA each yield `true`. Confirm every case exits 0, and that `actionlint` reports nothing for the new job beyond the two pre-existing `SC2012` findings in the staged Fargate step.
